...
export default function AdminOrderDetailPage() {
  ...
  const handleShip = async () => {
    ...
    try {
      ...
      toast.success('Order is now in transit for delivery')
    } catch (e) {
      toast.error(e.response?.data?.message || 'Cannot initiate shipment dispatch')
    } finally {
      setUpdating(false)
    }
  }
  ...
  const handleComplete = async () => {
    ...
    try {
      ...
      toast.error(e.response?.data?.message || 'Cannot complete order')
    } finally {
      setUpdating(false)
    }
  }
  ...
  const handleFulfill = async (vals) => {
    ...
    } catch (e) { toast.error(e.response?.data?.message || 'Cannot pack order') }
    ...
    <div className="space-y-5 animate-fade-in">
      ...
      <div className="card p-5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-4">
            ...
            <div>
              <p className="text-xs text-slate-500 font-medium uppercase mb-1 flex items-center gap-1">
                <PhoneOutlined className="text-sm" /> Phone Number
              </p>
              {order.customerPhone ? (
          ...
          <div className="space-y-4">
            <div>
              <p className="text-xs text-slate-500 font-medium uppercase mb-1">Shipping Address</p>
            ...
            <div>
              ...
              <Tag color={order.paymentStatus === 'DaThanhToan' ? 'green' : 'orange'} className="text-xs">
                {order.paymentStatus === 'DaThanhToan' ? 'Paid' : 'Pending Payment'}
              </Tag>
            </div>
          </div>
        </div>
      ...
      <div className="card p-5">
        ...
        <Descriptions bordered size="small" column={2}>
          <Descriptions.Item label="Total Amount"><strong>{formatCurrency(order.total)}</strong></Descriptions.Item>
          <Descriptions.Item label="Item Quantity"><strong>{order.itemsCount}</strong></Descriptions.Item>
        </Descriptions>
      </div>

      <div className="card p-5">
        <h3 className="font-semibold text-slate-800 mb-4">Order Line Items</h3>
        <div className="space-y-4">
