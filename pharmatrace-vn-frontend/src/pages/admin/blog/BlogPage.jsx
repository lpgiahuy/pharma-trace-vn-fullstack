...
export default function AdminBlogPage() {
  ...
  const handleDelete = async (id) => {
    try { await blogService.delete(id); toast.success('Article deleted successfully'); fetchData() }
  ...
  const cols = [
    ...
    { title: 'Published Date', dataIndex: 'publishedAt', key: 'date',  render: v => formatDate(v) },
    { title: 'Views', dataIndex: 'views',       key: 'views',  render: v => v?.toLocaleString() },
    {
      ...
      render: (_, row) => (
        <div className="flex gap-1">
          ...
          <Popconfirm title="Delete this article?" onConfirm={() => handleDelete(row.id)} okText="Delete" okButtonProps={{ danger: true }}>
  ...
  return (
    <div className="space-y-4 animate-fade-in">
      <div className="flex items-center justify-between">
        <div><h1 className="text-xl font-display font-bold text-slate-900">Blog Articles & Health News</h1></div>
        <AButton type="primary" icon={<PlusOutlined />} onClick={() => navigate('/admin/blog/new')}>New Article</AButton>
      </div>
